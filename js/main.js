...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $('.julius-category-filter').on('click', function(e) {
            ...
            const $searchInput = $('#julius-service-search');
            ...
            $('.julius-category-filter').removeClass('bg-primary text-primary-foreground').addClass('bg-secondary/50 text-muted-foreground hover:bg-secondary hover:text-foreground');
            $button.removeClass('bg-secondary/50 text-muted-foreground hover:bg-secondary hover:text-foreground').addClass('bg-primary text-primary-foreground');
        ...
        let searchTimeout;
        $('#julius-service-search').on('input', function() {
            const $input = $(this);
            ...
            $('.julius-category-filter').removeClass('bg-primary text-primary-foreground').addClass('bg-secondary/50 text-muted-foreground hover:bg-secondary hover:text-foreground');
            $('.julius-category-filter[data-category="all"]').removeClass('bg-secondary/50 text-muted-foreground hover:bg-secondary hover:text-foreground').addClass('bg-primary text-primary-foreground');
            
            // Debounce search - wait 300ms after user stops typing
            searchTimeout = setTimeout(function() {
                // Show loading state
                $container.html('<div class="text-center py-12"><svg class="animate-spin w-8 h-8 mx-auto text-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><p class="mt-4 text-muted-foreground">Searching...</p></div>');
                ...
                $.ajax({
                    ...
                    error: function() {
                        $container.html('<div class="text-center py-12"><p class="text-destructive">An error occurred. Please try again.</p></div>');
                    }
                });
            }, 300);
        });
